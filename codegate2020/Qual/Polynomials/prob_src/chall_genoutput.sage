...
def wrapper(N, p, q, pt, f=None, h=None):
    chall = Chall(N, p, q)
    chall.keygen()
    if DEBUG:
        chall.setPublicKey(h)
        chall.setPrivateKey(f)
    print(chall.getPublicKey())
    print(chall.encrypt(pt))
    if DEBUG:
        print(chall.count(list((chall.S(f) - 1) / p)))
    else:
...
if __name__ == "__main__":
    ...
    if DEBUG:
        from secret import f0, f1, h0, h1
        wrapper(55, 3, 4027, key1, f0, h0)
        wrapper(60, 3, 1499, key2, f1, h1)
    else:
